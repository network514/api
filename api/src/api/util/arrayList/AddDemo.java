...
public class AddDemo {
	public static void main(String[] args) {
		...
		System.out.println();
		/*
		 * 출력 2. listIterator() 이용 // 디자인패턴중 하나.
		 */
		ListIterator<String> it2 = lists.listIterator();
		while (it2.hasNext()) {
			System.out.printf("%s", it2.next());
			// 다운캐스팅이 필요없다. String이 될 예정이다.
		}
		System.out.println();
		/*
		 * 출력 3. previous() 이용
		 */
		while (it2.hasPrevious()) { //거의 안씀.
			// 메소드 명이 바뀐 부분 체크 !
		...
		//패턴입니다.
		list2.addAll(list2); // 서울, 부산, 대구를 복사해서 집어 넣은 상태
		/*
		 * 출력 4. 확장 for 문
		...
		while (it4.hasNext()) {
			...
			//참고로 printf에서 
			//10진수 숫자값은 %d 로 d는 decimal의 약자입니다.
		}
		System.out.println();
	}
}
